...
fn loyalty_key(env: &Env, addr: &Address) -> Symbol { Symbol::new(&[], "loyalty") }
...
#[contract] pub struct LoyaltyContract;
#[contractimpl]
impl LoyaltyContract {
    pub fn create_profile(env: Env, pioneer: Address) -> LoyaltyProfile {
        pioneer.require_auth();
        let p = LoyaltyProfile { pioneer: pioneer.clone(), points: 0, tier: LoyaltyTier::Starter, lifetime_points: 0, redeemable_points: 0, last_activity: env.ledger().timestamp(), referral_code: BytesN::from_array(&env, &[0;32]), referral_count: 0, activity_streak: 0 };
        env.storage().persistent().set(&loyalty_key(&env, &pioneer), &p); p
    }
    ...
    pub fn redeem_reward(env: Env, pioneer: Address, _reward_type: RewardType, amount: u32) {
        ...
        p.redeemable_points -= amount; env.storage().persistent().set(&loyalty_key(&env, &pioneer), &p);
    }
    pub fn get_profile(env: Env, pioneer: Address) -> LoyaltyProfile { env.storage().persistent().get(&loyalty_key(&env, &pioneer)).unwrap() }
}
